...
def trackfiles():
    ...
    for eachfile in projecthermes['inputs']:
        ...
        hexcode = sha1(filecontents.encode()).hexdigest()
...
# def trackdeltas():
#     global projecthermes, cwd, changedfiles

#     inputfiles = projecthermes['inputs']
#     tracks = dict()
...
#         try:
#             hexdata = None
#             itsheaders = getheaders(eachfile)
            
#             with open(eachfile, 'r') as file:
#                 hexdata = sha1(file.read().strip().encode()).hexdigest()
            
#             if eachfile in tracks.keys():
#                 if tracks[eachfile] != hexdata:
#                     changedfiles.append(eachfile)
#             else:
#                 changedfiles.append(eachfile)
            
#             filepath = '/'.join('/'.join(eachfile.split('\\')).split('/')[:-1])

#             for each in itsheaders:
#                 if each not in tracks.keys():
#                     hexdata = None
#                     with open(f'{cwd}', 'r') as file: hexdata = sha1(file.read().encode()).hexdigest()
#                     tracks[each] = {
#                         'hash' : hexdata,
#                         'files' : [eachfile]
#                     }
...
#         except FileNotFoundError:
#             print(colorama.Fore.RED + f"File not found! `{eachfile}` cannot be opened!" + colorama.Style.RESET_ALL)
#             return
